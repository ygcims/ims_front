  ...
  const fetchSourceDetails = async () => {
    try {
      const res = await axios.post(
        `${app_url}/datamanagement/getSelectedSource`,
        { id }
      );

      if (Array.isArray(res.data) && res.data.length > 0) {
        const sourceData = res.data[0];
        setInputs({
  ...
  const handleSubmit = async () => {
    try {
      if (id) {
        ...
        console.log(inputs);
        const res = await axios.post(
          `${app_url}/datamanagement/updateSource`,
          inputs
        );
      } else {
        const res = await axios.post(
          `${app_url}/datamanagement/addSource`,
          inputs
        );
      }
      swal("Success!", "Sourse added successfully.", "success").then(() => {
