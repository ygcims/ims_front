  ...
  const fetchCourseDetails = async () => {
    try {
      const res = await axios.post(
       `${app_url}/datamanagement/getSelectedCourse`,
        { id }
      );
      ...
      if (Array.isArray(res.data) && res.data.length > 0) {
        ...
        setInputs({
          name: courseData.name || "", // Use an empty string if name is null
          description: courseData.description || "", // Use an empty string if description is null
        });
      } else {
  ...
  const handleSubmit = async () => {
    try {
      if (id) {
        ...
        console.log(inputs);
        const res = await axios.post(
          `${app_url}/datamanagement/updateCourse`,
          inputs
        );
      } else {
        const res = await axios.post(
          `${app_url}/datamanagement/addCourse`,
          inputs
        );
      }
